...
UpdaterHttp::UpdaterHttp(QObject *parent) : QObject()
{
    Q_UNUSED(parent);
    m_url = "";
    m_openUrl = "";
 ...
 */
QString UpdaterHttp::openUrl() const {
    return m_openUrl;
}
 ...
 */
QString UpdaterHttp::changelog() const {
    return m_changelog;
}
 ...
 */
QString UpdaterHttp::moduleName() const {
    return m_moduleName;
}
 ...
 */
QString UpdaterHttp::platformKey() const {
    return m_platform;
}
 ...
 * \warning You should call \c checkForUpdates() before using this function
 */
QString UpdaterHttp::downloadUrl() const {
 ...
 * \warning You should call \c checkForUpdates() before using this function
 */
QString UpdaterHttp::latestVersion() const {
 ...
 */
QString UpdaterHttp::moduleVersion() const {
    return m_moduleVersion;
}

/**
 * Returns \c true if the UpdaterHttp should NOT interpret the downloaded appcast.
 * This is useful if you need to store more variables (or information) in the
 * JSON file or use another appcast format (e.g. XML)
 */
bool UpdaterHttp::customAppcast() const {
    return m_customAppcast;
}

/**
 * Returns \c true if the UpdaterHttp should notify the user when an update is
 * available.
 */
bool UpdaterHttp::notifyOnUpdate() const {
    return m_notifyOnUpdate;
}

/**
 * Returns \c true if the UpdaterHttp should notify the user when it finishes
 * checking for updates.
 *
 * \note If set to \c true, the \c UpdaterHttp will notify the user even when there
 *       are no updates available (by congratulating him/her about being smart)
 */
bool UpdaterHttp::notifyOnFinish() const {
 ...
 * \warning You should call \c checkForUpdates() before using this function
 */
bool UpdaterHttp::updateAvailable() const {
    return m_updateAvailable;
}

/**
 * Returns \c true if the integrated downloader is enabled.
 * \note If set to \c true, the \c UpdaterHttp will open the downloader dialog if
 *       the user agrees to download the update.
 */
bool UpdaterHttp::downloaderEnabled() const {
    return m_downloaderEnabled;
}

/**
 * Returns \c true if the UpdaterHttp shall not intervene when the download has
 * finished (you can use the \c QSimpleUpdaterHttp signals to know when the
 * download is completed).
 */
bool UpdaterHttp::useCustomInstallProcedures() const {
 ...
 * \c url() function.
 */
void UpdaterHttp::checkForUpdates() {
    m_manager->get (QNetworkRequest (url()));
}

/**
 * Changes the \c url in which the \c UpdaterHttp can find the update definitions
 * file.
 */
void UpdaterHttp::setUrl (const QString& url) {
    m_url = url;
}


void UpdaterHttp::setDownloadUrl (const QString& url) {
    m_downloadUrl = url;
}
 ...
 */
void UpdaterHttp::setModuleName (const QString& name) {
    m_moduleName = name;
}

/**
 * If \a notify is set to \c true, then the \c UpdaterHttp will notify the user
 * when an update is available.
 */
void UpdaterHttp::setNotifyOnUpdate (const bool notify) {
 ...
 */
void UpdaterHttp::setNotifyOnFinish (const bool notify) {
    m_notifyOnFinish = notify;
}

void UpdaterHttp::setLatestVersion(const QString &version)
{
 ...
 *       If the \a version parameter is empty, then the \c UpdaterHttp will use the
 *       application version (referenced by \c qApp)
 */
void UpdaterHttp::setModuleVersion (const QString& version) {
    m_moduleVersion = version;
}

/**
 * If the \a enabled parameter is set to \c true, the \c UpdaterHttp will open the
 * integrated downloader if the user agrees to install the update (if any)
 */
void UpdaterHttp::setDownloaderEnabled (const bool enabled) {
 ...
 *    - On Microsoft Windows: \c windows
 */
void UpdaterHttp::setPlatformKey (const QString& platformKey) {
    m_platform = platformKey;
}

/**
 * If the \a customAppcast parameter is set to \c true, then the \c UpdaterHttp
 ...
 */
void UpdaterHttp::setUseCustomAppcast (const bool customAppcast) {
    m_customAppcast = customAppcast;
}

/**
 * If the \a custom parameter is set to \c true, the \c UpdaterHttp will not try
 * to open the downloaded file. Use the signals fired by the \c QSimpleUpdaterHttp
 * to install the update from the downloaded file by yourself.
 */
void UpdaterHttp::setUseCustomInstallProcedures (const bool custom) {
 ...
 */
void UpdaterHttp::onReply (QNetworkReply* reply) {
 ...
 * settings of this instance of the \c UpdaterHttp class.
 */
void UpdaterHttp::setUpdateAvailable (const bool available) {
    ...
    if (updateAvailable() && (notifyOnUpdate() || notifyOnFinish())) {
        QString text = tr("您愿意现在下载更新吗？");
        QString title = "<h3>"
                        + tr ("圈图最新版本 %1 已经发布 !").arg (latestVersion())
                        + "</h3>";
 ...
 */
bool UpdaterHttp::compare (const QString& x, const QString& y) {
